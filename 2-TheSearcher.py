...
# TODO replace with a CSV reader or pandas
...
# TODO This seems computationally slow and error prone -VP
# count up the keyword matches
def find_keywords_in_file(file_path, keywords):
    """Counts the number of times a keyword appears in a text string

    Args:
        file_path (str): Path to the text file
        keywords (list[str]): List of keywords to search for in the

    Returns:
        dict(str, int): A dictionary mapping keyword to its counts in a document
    """
    try:
...
auth_file = (
    "/Users/jameschambers/Library/CloudStorage/Dropbox/LMF/pub/pub/www/users.csv"
)
...
# What this section does: Count up the total number of keywords in each document and multiply by 100 (but only if there's at least 2 keywords in that doc).
for filename in os.listdir(annualdir):
    ...
    if os.path.isfile(file_path):
        ...
        if termresults:
            ...
            # TODO So this only triggers if the count of keywords in the document is >=2. Is this intentional? -VP
            if termscore > 100:
                outputs.append(f"{articlenum}, {termscore}")
                # TODO: This is actually sorting the list of outputs each time you read a new article, which isn't necessary. Sort once at the end.
                # Additionally, unless there's something odd about 'articlenum', you don't need the key= section of the lambda function since the contents of outputs is strings.
                # Why would you sort by only the first 3 letters? -VP
                sorted_outs = sorted(outputs, key=lambda x: x[:3])
            else:
                bob = 1  # TODO What is this for? -VP
...
with open(annualsum, "r") as file:
    for line in file:
        ...
        if authscore > 1:
            ...
            # TODO: Just sort once at the end after you've collected everything -VP
